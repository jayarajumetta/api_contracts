...
class GitFinalizationAgent:
    ...
    def run(self):
        ...
        if not cfg.get("enabled",False):
            report["reason"]="git_finalization_disabled"; self.finish(report); return AgentResult(self.name,"success",0.9,report,report)
        if not cfg.get("execute_git",False):
            report["reason"]="execute_git_false"; self.finish(report); return AgentResult(self.name,"success",0.85,report,report)
        if not git_check.get("gitBinaryAvailable"):
            report["reason"]="git_binary_missing"; self.finish(report); return AgentResult(self.name,"failed_open",0.25,report,report)

        repo_url=cfg.get("repo_url","")
        token=os.environ.get(cfg.get("token_env","GIT_TOKEN"),"")
        username=os.environ.get(cfg.get("username_env","GIT_USERNAME"),"")
        if not repo_url:
            report["reason"]="repo_url_missing"; self.finish(report); return AgentResult(self.name,"failed_open",0.25,report,report)
        if not token:
            report["reason"]="git_token_missing"; self.finish(report); return AgentResult(self.name,"failed_open",0.25,report,report)
        ...
        try:
            ...
            if clone["returncode"]!=0:
                report.update({"reason":"git_clone_failed","cloneReturnCode":clone["returncode"],"cloneStdout":clone.get("stdout","")[-2000:],"cloneStderr":clone.get("stderr","")[-4000:],"likelyCause":self.likely_git_cause(clone.get("stderr","")+clone.get("stdout",""))})
                self.finish(report); return AgentResult(self.name,"failed_open",0.35,report,report)
            report["gitExecuted"]=True

            # Always base work branch from main/default base.
                ...
                if cfg.get("push",False):
                    ...
                    if push["returncode"]==0:
                        ...
                        if pr_cfg.get("enabled",False):
                            ...
                            report["pullRequest"]=pr; report["prCreated"]=bool(pr.get("created"))
                    else:
                        report.update({"reason":"git_push_failed","pushReturnCode":push["returncode"],"pushStdout":push.get("stdout","")[-2000:],"pushStderr":push.get("stderr","")[-4000:],"likelyCause":self.likely_git_cause(push.get("stderr","")+push.get("stdout",""))})
                else:
        ...
        self.finish(report)
        return AgentResult(self.name,"success" if not report.get("error") and report.get("reason") not in {"git_push_failed","git_clone_failed"} else "failed_open",0.95 if report.get("pushed") else 0.9 if report.get("committed") or report.get("reason") in {"no_changes_to_commit","execute_git_false","committed_locally_push_false"} else 0.4,report,report)

    def github_https_token_url(self,repo_url,token,username=""):
        if not repo_url.startswith("https://"): return repo_url
        user=quote(username or "x-access-token",safe="")
        tok=quote(token,safe="")
        return repo_url.replace("https://",f"https://{user}:{tok}@")

    def git_env(self):
        env=os.environ.copy(); env["GIT_TERMINAL_PROMPT"]="0"; return env
    ...
    def run_cmd(self,cmd,cwd=None,timeout=120,check=True,redact_token="",env=None):
        ...
        try:
            ...
            self.commands.append(item); self.ctx.write_json("git/command_log.json",{"commands":self.commands})
            if check and p.returncode!=0:
                raise RuntimeError(f"command failed: {safe_cmd} rc={p.returncode} stderr={item['stderr'][-1000:]}")
            return item
        except Exception as e:
            item.update({"exception":str(e)}); self.commands.append(item); self.ctx.write_json("git/command_log.json",{"commands":self.commands})
            if check: raise
            return item
    ...
    def likely_git_cause(self,text):
        t=(text or "").lower()
        if "authentication failed" in t or "could not read username" in t or "terminal prompts disabled" in t: return "authentication_failed_token_missing_invalid_or_not_accepted"
        if "permission" in t or "403" in t or "write access" in t: return "token_lacks_repo_write_permission"
        if "protected branch" in t or "gh006" in t: return "branch_protection_rejected_push"
        if "repository not found" in t: return "repo_not_found_or_token_no_access"
        if "src refspec" in t: return "branch_refspec_issue"
        if "non-fast-forward" in t: return "non_fast_forward_rejected"
        return "inspect_git_command_log"

    def create_pr(self,repo_url,token,head,base_branch,pr_cfg):
        if not pr_cfg.get("execute_network_calls",False): return {"created":False,"reason":"pr_execute_network_calls_false"}
        m=re.search(r"github\.com[:/](.+?)/(.+?)(?:\.git)?$",repo_url)
        if not m: return {"created":False,"reason":"unsupported_repo_url"}
        ...
        if body_file and Path(body_file).exists():
            try: body_text=Path(body_file).read_text(encoding="utf-8")
            except Exception: pass
        body=json.dumps({"title":pr_cfg.get("title","QAira generated API contracts and tests"),"body":body_text,"head":head,"base":base_branch}).encode()
        req=urllib.request.Request(url,data=body,headers={"Authorization":"Bearer "+token,"Accept":"application/vnd.github+json","Content-Type":"application/json"})
        try:
