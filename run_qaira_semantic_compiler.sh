...
if [ -f "$CONFIG_PATH" ]; then ARGS+=(--config "$CONFIG_PATH"); fi
if [ -n "$CHANGED_FILES" ] && [ -f "$CHANGED_FILES" ]; then ARGS+=(--changed-files "$CHANGED_FILES"); fi
python3 -m qaira_semantic_compiler.orchestrator_v63 "${ARGS[@]}"
